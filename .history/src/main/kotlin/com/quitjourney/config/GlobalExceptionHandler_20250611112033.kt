...
class GlobalExceptionHandler {
    ...
    fun handleValidationErrors(ex: MethodArgumentNotValidException): ResponseEntity<ApiResponse<Nothing>> {
        ...
        // 获取第一个错误作为主要错误信息
        val primaryError = errors.values.firstOrNull() ?: "输入数据验证失败"
        
        // 构建详细的错误信息
        val detailMessage = if (errors.size == 1) {
            primaryError
        } else {
            "$primaryError (共${errors.size}个错误)"
        }
        
        println("验证错误详情: $errors")
        println("返回错误信息: $detailMessage")
        
        return ResponseEntity
            .status(HttpStatus.BAD_REQUEST)
            .body(ApiResponse.error("VALIDATION_ERROR", detailMessage, errors))
    }
